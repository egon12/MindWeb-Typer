...
export default class DAG {
	...
	getLevel(level, done, notyet) {
		...
		const newnotyet = notyet.filter(
			i => !newdone.map(i => i.id).includes(i.id)
		);

		if (notyet.length == newnotyet.length)
			throw "Possible cyclic link in " + notyet.map(i => i.id).join(", ");
	...
	setXByLevel(data, maxLevel) {
		...
		for (let level in countItemPerLevel) {
			if (maxItem < countItemPerLevel[level])
				maxItem = countItemPerLevel[level];
		}
