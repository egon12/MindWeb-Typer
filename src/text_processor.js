export default function textProcessor(inputString) {
	...
	function isMm(pos) {
		return configurator.mm.indexOf(parseInt(pos)) !== -1;
	}
	function isMmStop(pos) {
		return configurator.mmstop.indexOf(parseInt(pos)) !== -1;
	}
	function addLinkToPos(pos, link) {
		basic_nodes[pos].link = basic_nodes[pos].link.concat(link);
	}
	...
	for (i in basic_nodes) {
		...
		if (isMmStop(i)) {
			depth--;
			if (depth < 0) {
				console.error("Wrong with something?");
			}
		}
	}
