export default class PositionCalculator {
	...
	createLinearScale(rangeFrom, rangeTo, domainFrom, domainTo) {
		const multiplier =
			(parseFloat(rangeTo) - parseFloat(rangeFrom)) /
	...
	createScale(data, config) {
		...
		return {
			x: this.createLinearScale(
				0,
				config.container.width - config.rect.width,
				minX,
				maxX
			),
			y: this.createLinearScale(
				0,
				config.container.height - config.rect.height,
				minY,
				maxY
			)
		};
	...
	createPosition(data, scale) {
		return data.map(i =>
			Object.assign({}, i, { r_x: scale.x(i.x), r_y: scale.y(i.y) })
		);
	}
	...
	createConnections(data, map, config) {
		...
		for (const i in data) {
			...
			for (const j in from.link) {
				...
				const color =
					link.color === undefined || link.color === ""
						? from.color
						: link.color;
